...
func cfg_entry_compile(_doc *Doc, _entry *Entry) ([]string, bool){
	...
	*/
	res = cfg_write_args(_doc, _entry, &args, &_entry.Flags, &_doc.Compiler.CompilePrefixGroup.FLG, false)
	if !res{
		return nil, false
	}
	res = cfg_write_args(_doc, _entry, &args, &_entry.Definitions, &_doc.Compiler.CompilePrefixGroup.DEF, false)
	if !res{
		return nil, false
	}
	res = cfg_write_args(_doc, _entry, &args, &_entry.LinkHooks, &_doc.Compiler.CompilePrefixGroup.INC, true)
	if !res{
		return nil, false
	}
	res = cfg_write_args(_doc, _entry, &args, &_entry.LinkFlags, &_doc.Compiler.CompilePrefixGroup.FLG, false)
	if !res{
		return nil, false
	}

	args = append(args, _doc.Compiler.CompilePrefixGroup.SRC)
	args = append(args, "")
	args = append(args, _doc.Compiler.CompilePrefixGroup.OUT)
	args = append(args, "")
	...
	*/
	filepath.Walk(_entry.BaseDirectory, func(path string, info fs.FileInfo, err error) error{
		...
		*/
		cmd := exec.Command(_doc.Compiler.Cmd, args...)
...
func cfg_entry_link(_doc *Doc, _entry *Entry, _sources []string) (string, bool){
	...
	case FILE_EXE:
	case FILE_SLIB:
		//cmd_exec, cmd_args, res = cfg_slib_cmd(_doc, _entry, _sources)
		if !res{
			return "", false
		}
	default:
	...
	tokens := strings.Fields(cmd_args)

	/*
		Form a command from builders and paths
	*/
	cmd := exec.Command(cmd_exec, tokens...)
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	cmd.Run()
